...
module.exports = {
    ...
    async exec(interaction) {
        ...
        const embed = new MessageEmbed().setColor("BLURPLE").setTitle("Définir le temps en heures").setDescription("Quelle intervalle de temps (en h) voulez-vous définir entre chaque commande `crime` ?");
                    ...
                    interaction.channel.awaitMessages({filter: filterAmount, max: 1, time: 60000, errors: ['time'] })
                        .then(async collected => {
                            ...
                            if (tag2?.features?.startsWith("on")) {
                                const strR = tag2.crime.split(",")[0] + "," + tag2.crime.split(",")[1] + "," + time;
                                
                                await TagsEconomyGuilds.update({crime: strR}, { where: { guildId: interaction.guild.id } });

                                const embed = new MessageEmbed().setColor("GREEN").setDescription(`:clock4: Le temps entre chaque commande \`crime\` a été défini sur ${time} heure(s)`);

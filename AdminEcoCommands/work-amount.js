...
module.exports = {
    ...
    async exec(interaction) {
        ...
        const filterAmount = response => {
            ...
            if (response.author.id === interaction.user.id && !isNaN(parseInt(montants[0])) && !isNaN(parseInt(montants[1])) && parseInt(montants[1]) != 0 && parseInt(montants[2]) != 0) {
        ...
        const embed = new MessageEmbed().setColor("BLURPLE").setTitle("Revenu minimum / Revenu maximum").setDescription("Veuillez indiquer le montant minimum (ne peut pas être négatif contrairement à `crime` et `rob`) et le montant maximum qu’il sera possible de gagner en faisant la commande `work` **séparés par une virgule**");
                ...
                .then(() => {
                    interaction.channel.awaitMessages({filter: filterAmount, max: 1, time: 60000, errors: ['time'] })
                        .then(async collected => {
                            ...
                            montants[0] = Math.abs(parseInt(montants[0]));
                            montants[1] = Math.abs(parseInt(montants[1]));
                            
                            const tag2 = await TagsEconomyGuilds.findOne({ where: { guildId: interaction.guild.id } });
                            
                            if (tag2?.features?.startsWith("on")) {
                                const strR = montants.toString() + "," + tag2.work.split(",")[2];

                                await TagsEconomyGuilds.update({work: strR}, { where: { guildId: interaction.guild.id } });

                                const embed = new MessageEmbed().setColor("GREEN").setDescription(`Le revenu obtensible grâce à la commande \`work\` est désormais compris entre <:NitsuCoin:984446683284910080> ${montants[0]} et <:NitsuCoin:984446683284910080> ${montants[1]}`);

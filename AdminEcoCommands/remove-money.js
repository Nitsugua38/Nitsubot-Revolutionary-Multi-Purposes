...
module.exports = {
    ...
    async exec(interaction) {
        ...
        const embed = new MessageEmbed().setColor("BLURPLE").setTitle("Membre / Montant").setDescription("À qui voulez-vous retirer quelle somme d’argent ? \nFormat: `<@membre> <montant>`")
                ...
                .then(() => {
                    interaction.channel.awaitMessages({filter: filterUserAmount, max: 1, time: 60000, errors: ['time'] })
                        .then(async collected => {
                            ...
                            const montant = parseInt(collected.first().content.substring(collected.first().content.indexOf(">")+1).replace("-", ""));
                            ...
                            if (!tag) {
                                if (tag2?.features?.startsWith("on")) {
                                    ...
                                    const embed = new MessageEmbed().setColor("GOLD").setDescription(`<:NitsuCoin:984446683284910080> ${montant} ont été retirés du cash de ${membre.displayName}`);

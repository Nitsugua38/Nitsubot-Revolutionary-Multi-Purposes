...
module.exports = {
    ...
    async exec(interaction) {
        ...
        const filterAmount = response => {
            ...
            if (response.author.id === interaction.user.id && !isNaN(parseInt(montants[0])) && !isNaN(parseInt(montants[1]))) {
        ...
        const embed = new MessageEmbed().setColor("BLURPLE").setTitle("Revenu minimum / Revenu maximum").setDescription("Veuillez indiquer le montant minimum et le montant maximum qu’il sera possible de gagner en envoyant des messages **séparés par une virgule**");
                    ...
                    interaction.channel.awaitMessages({filter: filterAmount, max: 1, time: 60000, errors: ['time'] })
                        .then(async collected => {
                            ...
                            if (tag2?.features?.startsWith("on")) {
                                const strR = montants.toString() + "," + tag2.chat.split(",")[2];

                                await TagsEconomyGuilds.update({chat: strR}, { where: { guildId: interaction.guild.id } });

                                const embed = new MessageEmbed().setColor("GREEN").setDescription(`Le revenu obtensible en envoyant des messages est désormais compris entre <:NitsuCoin:984446683284910080> ${montants[0]} et <:NitsuCoin:984446683284910080> ${montants[1]}`);

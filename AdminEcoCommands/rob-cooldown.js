...
module.exports = {
    ...
    async exec(interaction) {
        ...
        const embed = new MessageEmbed().setColor("BLURPLE").setTitle("Définir le temps en heures").setDescription("Quelle intervalle de temps (en h) voulez-vous définir entre chaque commande `rob` ?");
                    ...
                    interaction.channel.awaitMessages({filter: filterAmount, max: 1, time: 60000, errors: ['time'] })
                        .then(async collected => {
                            ...
                            if (tag2?.features?.startsWith("on")) {
                                ...
                                await TagsEconomyGuilds.update({rob: strR}, { where: { guildId: interaction.guild.id } });

                                const embed = new MessageEmbed().setColor("GREEN").setDescription(`:clock4: Le temps entre chaque commande \`rob\` a été défini sur ${time} heure(s)`);

...
module.exports = {
    ...
    async execute(interaction) {
        if (interaction.memberPermissions.has(Permissions.FLAGS.KICK_MEMBERS)) {
            ...
            interaction.reply(`${interaction.user} Tapez le message à envoyer à l’utilisateur à expulser : raison ou autres (Max : 1000 caractères)`)
                .then(() => {
                    interaction.channel.awaitMessages({ max: 1, time: 60000, errors: ['time'] })
                        .then(collected => {
                            ...
                            const embedok = new MessageEmbed().setColor('FFD700').setTitle(`${usertokick.username} a été expulsé`).setDescription(`par ${interaction.user}`).setAuthor('Log :', `${interaction.user.displayAvatarURL({dynamic: true})}`).setThumbnail(`${usertokick.displayAvatarURL({dynamic: true})}`).addField(':envelope: Message envoyé à l’utilisateur', `${messagetosend}`, true).setTimestamp().setFooter({ text : 'Messenger Bot. Made by Nitsugua38', iconURL: 'https://cdn.discordapp.com/avatars/917404523583135744/64302207180f2ef414df7cf89296e4ef.png'});
                            const embedtosend = new MessageEmbed().setColor('FFD700').setTitle(`:x: Vous avez été expulsé de ${interaction.guild.name}`).setThumbnail(`${interaction.guild.iconURL({dynamic: true})}`).addField(':envelope: Message : ', `${messagetosend}`, true).setTimestamp().setFooter({ text : 'Messenger Bot. Made by Nitsugua38', iconURL: 'https://cdn.discordapp.com/avatars/917404523583135744/64302207180f2ef414df7cf89296e4ef.png'});
                            targetMember.send({embeds: [embedtosend] }).catch(error => { interaction.followUp(":negative_squared_cross_mark: Impossible d’envoyer le message, l’utilisateur a bloqué les DM");})
                                .then(() => targetMember.kick().catch(error => { interaction.followUp(":x: Impossible d’expulser le membre. Vous ne pouvez pas expulser des admins."); noerroroccured = false;}))
        ...
        } else {interaction.reply(":negative_squared_cross_mark: Vous n’avez pas la permission d’expulser des gens, demandez à monter en grade et rééssayez !");};
    },
};
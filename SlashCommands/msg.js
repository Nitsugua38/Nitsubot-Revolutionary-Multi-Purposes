...
module.exports = {
    ...
    async execute(interaction) {
        if (interaction.memberPermissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) {
            const usertosend = interaction.options.getUser('membre');
            const messagetosend = interaction.options.getString('message').substring(0,3000);
            ...
            const sendrow = new MessageActionRow().addComponents(new MessageSelectMenu().setCustomId("sendoptionsselector").setPlaceholder("Veuillez sélectionner").addOptions([
                ...
                interaction.reply({ content: `${interaction.member}`, components: [sendrow], embeds: [embedchose], ephemeral: true})
                .then(() => {
            ...
            const embedcantkick = new MessageEmbed().setTitle("<:NitsuRedTickRound:977520171734401054> Permission manquante").setDescription("Gérer les messages").setColor("DARK_RED");

const { TagsEnterprisesUsers } = require("../index.js");
const { MessageEmbed, MessageActionRow, MessageButton } = require("discord.js")


module.exports = {
    EnterprisesCmdName: "reset-member",
                ...
                .then(() => {
                    interaction.channel.awaitMessages({filter: filterUser, max: 1, time: 60000, errors: ['time'] })
                        .then(async collected => {
                            ...
                            const embedSure = new MessageEmbed().setColor("RED").setTitle(`Êtes-vous sûr ?`).setDescription(`Veuillez confirmer que vous voulez réinitialiser le système d’entreprise de ${membre.displayName} !`);
                                    ...
                                    interaction.channel.awaitMessageComponent({filter: filterUser2, componentType: "BUTTON", max: 1, time: 60000, errors: ['time'] })
                                        .then(async interaction => {
                                            if (interaction.customId === "aaECORESETyes") {
                                                await TagsEnterprisesUsers.destroy({ where: { GUID: GUID } });
                                                const embedDONE = new MessageEmbed().setColor("DARK_RED").setTitle(`Le système d’entreprise de ${membre.displayName} a bien été réinitialisé !`);

...
module.exports = {
    EnterprisesCmdName: "end-shortage",
    async exec(interaction) {
        ...
        if (tag2?.features?.startsWith("on")) {
            ...
            if (tag2.materials && tag2.materials !== "") {
                ...
                const embed = new MessageEmbed().setColor("DARK_VIVID_PINK").setTitle("Matériaux").setDescription("Veuillez choisir un matériau à mettre en état de production");
                ...
                .then(() => {
                    interaction.channel.awaitMessageComponent({ componentType: 'SELECT_MENU', filter: filterUser, max: 1, time: 60000, errors: ['time'] })
                        .then(async interaction => {
                            ...
                            const embedReply = new MessageEmbed().setColor("GREEN").setTitle("<:NitsuGreenTickRound:977520117216862239> Production relancée").setDescription(`Les entreprises produisent à nouveau le matériau ${itemName} !`);
